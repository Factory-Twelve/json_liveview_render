defmodule JsonLiveviewRender.Companion.ChatCards.Platform.WebChat do
  @moduledoc """
  Internal companion renderer for chatbot-style web chat view-model payloads.
  """
  ...
  def render(ir, _opts) do
    payload = %{
      "role" => "assistant",
      "message" => %{"text" => IR.message_text(ir)},
      "card" => %{
        "id" => ir.card_id,
        "title" => ir.title,
        ...
        "actions" =>
          Enum.map(ir.actions, fn action ->
            %{"id" => action.action_id, "label" => action.label, "style" => action.style}
          end)
      }
    }
