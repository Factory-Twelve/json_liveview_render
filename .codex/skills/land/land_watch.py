...
async def get_timeline_events(pr_number: int) -> list[dict[str, Any]]:
    page = 1
...
def filter_codex_comments(
    ...
    codex_comments = [
        c
        for c in comments
        if is_codex_bot_user(c.get("user", {}))
        and not is_codex_approval_body((c.get("body") or "").strip())
    ]
...
def is_codex_approval_body(body: str) -> bool:
    return any(body.startswith(prefix) for prefix in CODEX_APPROVAL_PREFIXES)
...
async def fetch_review_context(
    ...
    timeline_events = await get_timeline_events(pr_number)
    return issue_comments, review_comments, reviews, timeline_events, review_request_at


def codex_approved_via_reaction(
    timeline_events: list[dict[str, Any]],
    review_requested_at: datetime | None,
) -> bool:
    for event in timeline_events:
        if event.get("event") != "reacted":
            continue
        actor = event.get("actor") or {}
        if not is_codex_bot_user(actor):
            continue
        if event.get("content") != "+1":
            continue
...
def codex_approved(
    ...
    for comment in issue_comments + review_comments:
        if not is_codex_bot_user(comment.get("user", {})):
            continue
        body = (comment.get("body") or "").strip()
        if not is_codex_approval_body(body):
            continue
        created_time = comment_time(comment)
        if created_time is None:
            continue
        if review_requested_at is not None and created_time <= review_requested_at:
            continue
        return True
    return codex_approved_via_reaction(timeline_events, review_requested_at)
...
async def wait_for_codex(pr_number: int, checks_done: asyncio.Event) -> None:
    ...
    while True:
        ...
        if approval_seen:
            ...
            continue
        if review_request_at is None and checks_done.is_set():
            return
        await asyncio.sleep(POLL_SECONDS)
    ...
    while True:
        ...
        if not check_runs:
            ...
            if empty_seconds >= CHECKS_APPEAR_TIMEOUT_SECONDS:
                print("No GitHub checks detected; treating PR as check-clean")
                checks_done.set()
                return
            await asyncio.sleep(POLL_SECONDS)
            continue
