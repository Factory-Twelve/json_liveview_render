...
- Do not enable GitHub's native auto-merge blindly; use this land loop instead
  so local validation + Codex Connector feedback are handled before merge.
- If no GitHub checks are configured for the repo, treat the PR as check-clean
  after local validation is green and Codex Connector has passed.
...
- Treat either of these as a clean Codex Connector pass signal: (1)
  `chatgpt-codex-connector` reacts with 👍 on the PR, or (2) it leaves an
  issue/review comment that starts with `Codex Review: Didn't find any major issues.`
